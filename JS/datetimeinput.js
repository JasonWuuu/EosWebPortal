//  主调用函数是 setday(this,[object])和setday(this)，[object]，setday(this,[object]，2/1/0)是控件输出的控件名，举两个例子：
...
strFrame='<style>'; 
strFrame+='INPUT.button{BORDER-RIGHT: RGB(189,235,238) 1px solid;BORDER-TOP: RGB(189,235,238) 1px solid;BORDER-LEFT: RGB(189,235,238) 1px solid;'; 
strFrame+='BORDER-BOTTOM: RGB(189,235,238) 1px solid;BACKGROUND-COLOR:black;font-family:宋体;}'; 
...
strFrame+='<table border=1 cellspacing=0 cellpadding=0 width=142 height=160 bordercolor=RGB(189,235,238) bgcolor=RGB(189,235,238) >';
...
strFrame+='                <td width=16 align=center bgcolor=RGB(189,235,238) style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzPrevM()" title="向前翻 1 月" ><b >&lt;</b></td>';
strFrame+='				   <td width=60 align="center" bgcolor="RGB(189,235,238)"  style="font-size:12px;cursor:hand" ';
strFrame+='				       onmouseover="style.backgroundColor=\'RGB(192,235,239)\'"';
strFrame+='					   onmouseout="style.backgroundColor=\'RGB(189,235,238)\'" ';
...
strFrame+='					   bgcolor="RGB(189,235,238)" onmouseover="style.backgroundColor=\'RGB(192,235,239)\'" ';
strFrame+='					   onmouseout="style.backgroundColor=\'RGB(189,235,238)\'" ';
...
strFrame+='				   <td width=16 bgcolor=RGB(189,235,238) align=center style="font-size:12px;cursor: hand;color: #ffffff" ';
strFrame+='					   onclick="parent.meizzNextM()" title="向后翻 1 月" ><b >&gt;</b></td>';
strFrame+='			   </tr>';
strFrame+='		   </table></td></tr>';
strFrame+='	   <tr><td width=142 height=18 >';
strFrame+='		   <table border=0 cellspacing=0 cellpadding=2 bgcolor=RGB(189,235,238) ' + (bMoveable? 'onmousedown="DragStart()" onmouseup="DragEnd()"':'');
strFrame+='				BORDERCOLORLIGHT=RGB(189,235,238) BORDERCOLORDARK=#FFFFFF width=140 height=20  style="cursor:' + (bMoveable ? 'move':'default') + '">';
...
strFrame+='    <table border=1 cellspacing=2 cellpadding=2 BORDERCOLORLIGHT=RGB(189,235,238) BORDERCOLORDARK=#FFFFFF bgcolor=#ffffff width=140 height=120 >';
...
strFrame+='        <table border=0 cellspacing=1 cellpadding=0 width=100%  bgcolor=#FFFFFF height=22 id=table_time>';
strFrame+='          <tr bgcolor="RGB(189,235,238)"><td id=bUseTimeLayer width=30';
//caozhongguo
//strFrame+='				onmouseover="style.backgroundColor=\'RGB(192,235,239)\'" align=center onmouseout="style.backgroundColor=\'RGB(189,235,238)\'"';
...
strFrame+='             <td ';
//caozhongguo
strFrame+=' onmouseover="style.backgroundColor=\'RGB(192,235,239)\'" onmouseout="style.backgroundColor=\'RGB(189,235,238)\'"';
...
function setday(tt,obj,opt) //主调函数
{
	...
	{
      bUseTime=true;    
	  odatelayer.table_time.style.display="none";	
    }
	...
	while (tt = tt.offsetParent){ttop+=tt.offsetTop-80; tleft+=tt.offsetLeft;}
	ttop= (ttyp=="image") ? ttop+thei : ttop+thei+6;
	//lisongmao +
	if(ttop<10)
	ttop=10;
	
		var pwith=document.body.clientWidth;
	if((tleft+170)>pwith)
	{
		tleft=pwith-170;
	}
	...
	/*
	var tmpvalue=new Date();
	var comvalue=tmpvalue.getFullYear()+"-"+(tmpvalue.getMonth()+1)+"-"+tmpvalue.getDate()+" "+tmpvalue.getHours()+":"+tmpvalue.getMinutes()+":"+tmpvalue.getSeconds();
	if(CompareDate(outObject.value,comvalue))
	{
		outObject.value=comvalue;
	}
	*/
	
	//根据当前输入框的日期显示日历的年月
	//var reg = /^(\d+)-(\d{1,2})-(\d{1,2})/;		//不含时间
	var reg = /^(\d+)-(\d{1,2})-(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})/;		//含时间
	...
	if(r!=null){
		...
		if(d.getFullYear()==r[1] && d.getMonth()==r[2] && d.getDate()==r[3])
		{
			...
			//时分秒
			parent.meizzTheHour = r[4];
			parent.meizzTheMinute = r[5];
			parent.meizzTheSecond = r[6];
		}
		else
		{
			outDate="";
...
function meizzSetDay(yy,mm)	//主要的写程序**********
{
	...
	if (meizzWDay[i]!="")
	{
		...
		da.style.borderRightColor="RGB(189,235,238)"; 
		da.style.borderBottomColor="RGB(189,235,238)";
                ...
                if(!outDate) 
                        da.style.backgroundColor = ((mm==1?yy-1:yy) == new Date().getFullYear() && 
                                (mm==1?12:mm-1) == new Date().getMonth()+1 && meizzWDay[i] == new Date().getDate()) ? 
                                    "RGB(192,235,239)":"#ffffff"; 
                else 
                { 
                        da.style.backgroundColor =((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 && 
                        meizzWDay[i]==outDate.getDate())? "#00ffff" : 
                        (((mm==1?yy-1:yy) == new Date().getFullYear() && (mm==1?12:mm-1) == new Date().getMonth()+1 && 
                        meizzWDay[i] == new Date().getDate()) ? "RGB(192,235,239)":"#ffffff"); 
                        //将选中的日期显示为凹下去 
                        if((mm==1?yy-1:yy)==outDate.getFullYear() && (mm==1?12:mm-1)== outDate.getMonth() + 1 && 
                        meizzWDay[i]==outDate.getDate()) 
                        { 
                                da.style.borderTopColor="RGB(189,235,238)"; 
                                da.style.borderRightColor="#FFFFFF"; 
                                da.style.borderBottomColor="#FFFFFF"; 
                                da.style.borderLeftColor="RGB(189,235,238)"; 
                                da.style.backgroundColor= "RGB(192,235,239)";
                        } 
                        da.style.backgroundColor= "RGB(192,235,239)";
                } 
        } 
...
function meizzDayClick(n,ex)	//点击显示框选取日期，主输入函数*************
{
	...
	//caozhongguo
	if (mm < 10)	{mm = "0" + mm;}
	if (hh<10&&hh.Length<2)		{hh="0" + hh;}	//时
	if (mi<10&&mi.Length<2)		{mi="0" + mi;}	//分
	if (se<10&&se.Length<2)		{se="0" + se;}	//秒
	...
	var firstday = new Date(yy,mm-1,1).getDay()
	 firstday=parseInt(firstday)+parseInt(n1)-1;
	 var da = eval("odatelayer.meizzDay"+firstday);
	 if(ex==0)
	 {
       da.style.backgroundColor= "#00FF00";
     }
...
function WriteDateTo(yy,mm,n,hh,mi,se)
{
	if (bUseTime)
	{
		//caozhongguo
		var tmpvalue=new Date();
		var comvalue1=tmpvalue.getFullYear()+"-"+format(tmpvalue.getMonth()+1)+"-"+format(tmpvalue.getDate())+" "+format(tmpvalue.getHours())+":"+format(tmpvalue.getMinutes())+":"+format(tmpvalue.getSeconds());
		var comvalue2=yy+"-"+mm+"-"+n+" "+hh+":"+mi+":"+se;

		if(compare==1)
		{
		//start:与当前时间进行比较大于当前时间则取当前时间
		   if(CompareDate(comvalue2,comvalue1))
		   {
			  yy=tmpvalue.getFullYear();
			  mm=	tmpvalue.getMonth()+1;
			  n=	tmpvalue.getDate();	
			  hh=	tmpvalue.getHours();	
			  mi=	tmpvalue.getMinutes();	
			  se=	tmpvalue.getSeconds();	
		   }
		//end:
		
		//当SETDAY()的第3个参数为1时自动处理当开始时间大于结束时间，结束时间自动取开始时间
//		 if(CompareDate(document.all.tb_StartTime.value,document.all.tb_EndTime.value))
//		    {
//			   outObject.value=comvalue1;		
//		     }
